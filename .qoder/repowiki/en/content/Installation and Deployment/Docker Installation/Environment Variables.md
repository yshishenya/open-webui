...
- ENABLE_BILLING_WALLET / ENABLE_BILLING_SUBSCRIPTIONS
  ...
  - Purpose: Enable PAYG wallet billing and/or subscription billing.
  - Default: Wallet enabled; subscriptions disabled.
  - Impact: Disabling wallet removes PAYG wallet flows; disabling subscriptions removes plan/subscription flows.

- YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY, YOOKASSA_WEBHOOK_SECRET
  - Purpose: Payment provider integration and webhook signature verification.
  - Default: Not set by default; required when taking payments.
  - Impact: Missing values prevent payment processing and/or signature verification.
...
Cfg-->>Bill : "ENABLE_BILLING_WALLET, ENABLE_BILLING_SUBSCRIPTIONS, LEAD_MAGNET_ENABLED, YOOKASSA_*"
...
### Billing and Payments (ENABLE*BILLING_WALLET/ENABLE_BILLING_SUBSCRIPTIONS, YOOKASSA*\*)

- Definition and defaults
  - docker-compose enables billing by default and passes billing-related environment variables.
  - Payment creation requires `YOOKASSA_SHOP_ID` + `YOOKASSA_SECRET_KEY`; webhook signature verification optionally uses `YOOKASSA_WEBHOOK_SECRET`.
- Behavior
  - When billing is enabled, the backend initializes billing services and processes webhooks.
- Impact
  - Missing YooKassa credentials prevent payment creation; missing webhook secret disables signature verification.
...
ENABLE_BILLING_WALLET["ENABLE_BILLING_WALLET"] --> CFG
ENABLE_BILLING_SUBSCRIPTIONS["ENABLE_BILLING_SUBSCRIPTIONS"] --> CFG
LEAD_MAGNET_ENABLED["LEAD_MAGNET_ENABLED"] --> CFG
...
# Webhook URL is configured in YooKassa dashboard (not a backend env var)
```
...
- Billing webhook not processed
  ...
  - Resolution: Ensure wallet/subscriptions are enabled as needed (`ENABLE_BILLING_WALLET` / `ENABLE_BILLING_SUBSCRIPTIONS`) and `YOOKASSA_SHOP_ID` + `YOOKASSA_SECRET_KEY` are set. Verify the webhook endpoint is configured as `/api/v1/billing/webhook/yookassa` and set `YOOKASSA_WEBHOOK_SECRET` if you use signature verification.
...
- Development (local)
  ...
  - Keep billing flags disabled (wallet/subscriptions) unless you intend to run billing features.
...
- Production (standalone)
  ...
  - Enable billing only if integrating with a payment provider; set `YOOKASSA_*` accordingly and configure the webhook endpoint as `/api/v1/billing/webhook/yookassa`.
