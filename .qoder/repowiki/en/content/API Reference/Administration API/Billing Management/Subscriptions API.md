...
participant API
participant BillingService
YooKassa->>API : POST /api/v1/billing/webhook/yookassa
...
const paymentResponse = await fetch('/api/v1/billing/payment', {
  method: 'POST',
  headers: {
...
const cancelResponse = await fetch('/api/v1/billing/subscription/cancel', {
	method: 'POST',
	headers: {
		Authorization: `Bearer ${token}`,
		'Content-Type': 'application/json'
...
const billingInfo = await fetch('/api/v1/billing/me', {
	headers: {
		Authorization: `Bearer ${token}`
	}
});
...
## Trial Period Management (Optional)

Trial periods exist in the subscription implementation (see `trial_days` / `trial_end` in `backend/open_webui/utils/billing.py`), but Airis deployments are typically PAYG-first and rely on lead-magnet quotas (if enabled) rather than subscription trials.
...
3. Keep `current_period_end` set to the plan interval end (the current code does not shorten the billing period to the trial end)
4. During trial, access checks treat `TRIALING` similarly to `ACTIVE`

### Trial Period Endpoints

The system manages trial fields through the standard subscription creation/update paths; there are no dedicated “trial” endpoints.

### Trial Period Considerations

- This is subscription-only and optional in Airis
- Usage during a trial still counts toward quota checks
- Product copy for B2C should stay PAYG-first unless subscription plans are intentionally enabled
