...
symptoms:
  ...
  - "Task updates had unprocessed `meta/memory_bank/branch_updates/*` entries on integration branch."
...
| Grep docs for command drift | Different docs may show different compose syntax | ✅ Found `docker-compose` usage in `meta/docs/guides/billing_setup.md` while the repo standard is `docker compose` |

## Fix

- Updated Memory Bank docs to remove `Any` from code examples and align wording with AGENTS rules.
- Consolidated the completed branch update entry into `meta/memory_bank/current_tasks.md` and removed the processed `branch_updates` file.
- Standardized Docker Compose command examples to `docker compose` in Airis billing setup docs.
- Added a sharded Airis docs section under `meta/docs/` to reduce future doc sprawl.

## Verification

- Ran a local markdown-relative-link check (no broken links):
  - `python3 meta/tools/check_markdown_links.py`
