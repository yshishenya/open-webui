...
Documentation and `meta/memory_bank/` drifted over time:

- Examples contradicted project rules (e.g. `Any` in Python type-hint snippets).
- Integration-branch task consolidation left a completed `meta/memory_bank/branch_updates/*` entry unmerged.
...
- [x] Airis-specific docs are easy to discover and navigable (sharded docs under `meta/docs/`).
- [x] Add a durable place to record “how we fixed it” for future reuse (`meta/docs/solutions/`).
...
- Docs:
  - `meta/memory_bank/*` consistency updates
  - `meta/docs/*` (new)
  - `meta/docs/solutions/*` (new)
...
- Key files/entrypoints:
  ...
  - `meta/memory_bank/patterns/api_standards.md`
  - `meta/memory_bank/workflows/code_review.md`
  - `meta/memory_bank/current_tasks.md`
  - `meta/docs/README.md`
  - `meta/docs/solutions/documentation/2026-02-05__memory-bank-docs-consistency.md`
- API changes: none
- Edge cases:
...
- Upstream-owned files touched:
  - None (additive docs only under `meta/docs/`; existing upstream docs left intact).
- Why unavoidable:
  - N/A
...
- Docs link integrity: `python3 meta/tools/check_markdown_links.py` (0 broken links).
- Tests: N/A (docs-only).
