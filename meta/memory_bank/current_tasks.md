...
**Worktree/branch rule:** do **not** edit this file on non-integration branches (`feature/*`, `bugfix/*`, `refactor/*`, `docs/*`, `codex/*`). Use `meta/memory_bank/branch_updates/*` and consolidate on the integration branch per `meta/memory_bank/guides/task_updates.md`.

For non-trivial work items, each entry should include a `Spec:` link to a work item spec under `meta/memory_bank/specs/work_items/` (see `meta/memory_bank/specs/README.md`).

**Entry format rule (copy/paste safe):**

- Use the same bullet structure as `meta/memory_bank/branch_updates/_template.md`.
...
- [x] **[BUG][AUTH][UI]** Fix `/auth` social providers row (Telegram/VKID/OK/Mail)
  ...
  - Tests: `docker compose -f docker-compose.yaml -f docker-compose.dev.yaml run --rm --no-deps airis-frontend sh -lc "npm run test:frontend -- --run"`, `docker compose -f docker-compose.yaml -f docker-compose.dev.yaml run --rm --no-deps -e NODE_OPTIONS=--max-old-space-size=4096 airis-frontend sh -lc "npm run build:vite"`
  - Risks: Low (UI-only; verify provider buttons in prod/mobile).
...
- [x] **[DOCS]** Documentation + Memory Bank consistency cleanup
  - Spec: `meta/memory_bank/specs/work_items/2026-02-05__docs__docs-memory-bank-consistency-cleanup.md`
  - Owner: Codex
  - Done: 2026-02-05
...
- [x] **[DOCS][BILLING]** Wallet (/billing/balance) UX audit + improvement backlog
  - Spec: `meta/memory_bank/specs/work_items/2026-02-05__docs__billing-balance-ux-audit.md`
  - Owner: Codex
  - Done: 2026-02-05
...
- [x] **[DEV]** SDD toolkit: first-time project setup
  - Spec: `meta/memory_bank/specs/work_items/2026-02-05__docs__sdd-toolkit-setup.md`
  - Owner: Codex
  - Done: 2026-02-05
...
- [x] **[LEGAL]** Publish оферта + acceptance tracking
  - Spec: `meta/memory_bank/specs/work_items/2026-02-05__feature__legal-offer-docs-acceptance.md`
  - Owner: Codex
  - Done: 2026-02-05
...
- [x] **[UI]** Admin billing models: audit-first pricing
  - Spec: `meta/memory_bank/specs/work_items/2026-02-04__feature__admin-models-pricing-audit-first.md`
  - Owner: Codex
  - Done: 2026-02-04
...
- [x] **[BUG]** Rate card XLSX import: provider-only models + i18n placeholders
  - Spec: `meta/memory_bank/specs/work_items/2026-02-03__bugfix__rate-card-xlsx-import-provider-models-i18n.md`
  - Owner: Codex
  - Done: 2026-02-03
...
- [x] **[BUG]** YooKassa webhook: docs-aligned source verification (IP allowlist)
  - Spec: `meta/memory_bank/specs/work_items/2026-02-03__bugfix__yookassa-webhook-source-verification.md`
  - Owner: Codex
  - Done: 2026-02-03
...
- [x] **[UI]** Simplify billing pages UX (Wallet/History/Plans)
  ...
  - Spec: `meta/memory_bank/specs/billing_pages_ux_simplification.md`
...
- [x] **[BILLING]** Update rate card prices from competitor analysis
  ...
  - Output: `meta/memory_bank/images/rate-cards-all_units_template (2)-filled.xlsx`
  - **Owner**: Codex
  - **Done**: 2026-02-02

- [x] **[BUG]** Fix zero-priced rate cards (import + seed)
  - Updated `meta/memory_bank/images/rate-cards-all_units_template (2)-filled.xlsx` to remove active zero prices (fill missing token/image/audio rates; disable irrelevant modalities).
...
- [x] **[BUG][AUTH]** GitHub OAuth не отображается в UI при запуске через `docker-compose.yaml`
  ...
  - **Fix**: добавлены GitHub OAuth env vars в `docker-compose.yaml`, обновлены `.env.example` и `meta/docs/reference/b2c_implementation.md`
  - **Owner**: Codex
  - **Done**: 2026-02-02
...
- [x] **[TOOLS][ENV]** Скрипт для безопасной синхронизации `.env` по актуальному `.env.example`
  ...
  - **Fix**: добавлен `scripts/sync_env.py` (создаёт `.env.bak.<timestamp>` и печатает только названия ключей), документация в `meta/docs/reference/b2c_implementation.md`
  - **Owner**: Codex
  - **Done**: 2026-02-02
...
- [x] **[UI-28]** Implement billing wallet UX redesign (wallet-first)
  ...
  - Spec: `meta/memory_bank/specs/billing_wallet_ux_redesign.md`
  - **Owner**: Codex
  - **Done**: 2026-02-01
...
- [x] **[DOCS-10]** Create 2026 public pages plan (per-page wording + imagery)
  ...
  - Added `meta/memory_bank/specs/public_pages_2026_plan.md` with step-by-step blocks and copy
...
- [x] **[DOCS-08]** Document landing conversion plan from expert reviews
  ...
  - Added `meta/memory_bank/specs/landing_conversion_plan.md` with phased roadmap and open questions
...
- [x] **[BILLING-07]** Define PAYG default + lead magnet access logic
  ...
  - Spec saved: `meta/memory_bank/specs/payg_lead_magnet_policy.md`
  - **Owner**: TBD
  - **Target**: TBD
