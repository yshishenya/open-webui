...
kit-version: 0.1.0
template: scaffolds/memory_bank/guides/testing_strategy.md.tmpl
-->
...
- `pr-fast` (PR to `airis_b2c`): backend billing critical pack + billing balance frontend spec + billing wallet E2E pack.
- `merge-medium` (push to `airis_b2c`): `pr-fast` + backend billing coverage pack with module-scoped line+branch gate.
