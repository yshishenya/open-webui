...
- Перед реализацией создать рабочую ветку по workflow (например, `feature/yandex-oauth-login`) и перенести
  обновления в `.memory_bank/branch_updates/<YYYY-MM-DD>-feature-yandex-oauth-login.md` (worktree-safe).
...
- Docs sanity:
  - Ensure `.qoder/RUSSIAN_OAUTH_SETUP.md` and `README-B2C-IMPLEMENTATION.md` remain accurate (no behavior drift).
