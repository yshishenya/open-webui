...
- Удалить endpoint `/billing/estimate` на бэке.
...
3. Backend: удалить endpoint для UI-прогноза.
   - `backend/open_webui/routers/billing.py`: удалить `/billing/estimate` и связанные модели запроса/ответа (если не используются в других местах).
4. Тесты: удалить/обновить тесты, которые завязаны на `/billing/estimate`.
...
- Нет endpoint `/billing/estimate` и связанных типов в API клиента.
