...
- Lead magnet quotas exist (free-start usage on allowlisted models).
...
- `GET /api/v1/billing/public/lead-magnet` returns enabled, cycle_days, quotas (raw units).
...
- `GET /api/v1/billing/public/rate-cards` returns latest effective rate cards (token_in/out, image, tts, stt).
