...
- UI-элементы “Free plan activation” и PAYG как подписку (в пользовательском списке планов).
- Публичная витрина, показывающая PAYG как подписку (заменить на “Кошелёк”).
- Пользовательский роут `/billing/plans` (скрыть/редирект), если подписки выключены.
