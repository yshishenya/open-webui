...
from open_webui.utils.airis.safe_get import deep_get_mapping
...
async def generate_direct_chat_completion(
    ...
    if form_data.get('stream'):
        ...
        res = coerce_direct_ack(
            res,
            context=DirectAckContext(request_id=request_id, channel=channel),
        )
...
async def generate_chat_completion(
    ...
    if hasattr(request.state, 'metadata'):
        metadata_value = form_data.get('metadata')
        metadata = metadata_value if isinstance(metadata_value, dict) else {}
        state_metadata_value = request.state.metadata
        state_metadata = state_metadata_value if isinstance(state_metadata_value, dict) else {}
        form_data['metadata'] = {**metadata, **state_metadata}
    ...
    if getattr(request.state, 'direct', False) and model_id == getattr(request.state, 'model', {}).get('id'):
        ...
        if not selected_model_id and model.get('owned_by') == 'arena':
            model_meta = deep_get_mapping(model, ('info', 'meta'))
        ...
        if model.get('owned_by') == 'ollama':
            ...
            try:
                ...
                log.exception('Ollama billing preflight failed: %s', exc)
                raise HTTPException(
