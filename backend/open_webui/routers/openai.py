...
async def speech(request: Request, user=Depends(get_verified_user)):  # noqa: C901
    ...
    try:
        ...
        requested_billing_model = payload.get('model')
        billing_model = (
            requested_billing_model if isinstance(requested_billing_model, str) and requested_billing_model else 'tts-1'
        )
        ...
        try:
            ...
            log.exception('Billing preflight failed for OpenAI speech: %s', e)
            raise HTTPException(
        ...
        tmp_file_path = SPEECH_CACHE_DIR.joinpath(f'{name}.{suffix}.mp3.tmp')
        tmp_body_path = SPEECH_CACHE_DIR.joinpath(f'{name}.{suffix}.json.tmp')
        try:
            session = await get_session()
            ...
            async with aiofiles.open(tmp_body_path, 'w') as f:
                await f.write(json.dumps(payload))

            await asyncio.to_thread(tmp_body_path.replace, file_body_path)
            await asyncio.to_thread(tmp_file_path.replace, file_path)
            ...
            await asyncio.to_thread(tmp_body_path.unlink, missing_ok=True)
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='Airis: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    Airis stores extra bookkeeping fields (``id``, ``status``,
...
async def generate_chat_completion(  # noqa: C901
    ...
    try:
        messages_value = billing_payload.get('messages', [])
        billing_messages = messages_value if isinstance(messages_value, list) else []
        estimated_tokens = estimate_tokens_from_messages(billing_messages, model_id=model_id)
        ...
        log.exception('Billing preflight failed for OpenAI chat: %s', e)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail='Billing system temporarily unavailable',
        )
