...
from open_webui.utils.airis.image_billing import (
    ...
    release_image_billing,
    settle_image_billing,
)
...
async def image_generations(
    ...
    try:
        billing_context = await preflight_image_billing(
            ...
            provider=str(image_config.IMAGE_GENERATION_ENGINE or 'automatic1111'),
            operation='generate',
            requested_count=form_data.n,
            width=width,
            height=height,
            auto_size=(form_data.size == 'auto' or image_config.IMAGE_SIZE == 'auto'),
        )
    except HTTPException:
        raise
    except Exception as e:
        log.exception('Image billing preflight failed: %s', e)
        raise HTTPException(
            status_code=503,
    ...
    try:
        if image_config.IMAGE_GENERATION_ENGINE == 'openai':
            ...
            for image in res['images']:
                ...
                images.append({'url': url})
            return await finalize(images)
        raise HTTPException(status_code=400, detail='Unsupported image generation engine')
    except asyncio.CancelledError:
        await release_image_billing(billing_context)
        raise
    except HTTPException:
        await release_image_billing(billing_context)
        raise
...
async def image_edits(
    ...
    try:
        billing_context = await preflight_image_billing(
            user_id=user.id,
            model_id=model,
            provider=str(image_config.IMAGE_EDIT_ENGINE),
            operation='edit',
            requested_count=form_data.n,
            width=width,
            height=height,
        ...
        log.exception('Image edit billing preflight failed: %s', e)
        raise HTTPException(
            status_code=503,
            detail='Billing system temporarily unavailable',
        )
    ...
    try:
        if image_config.IMAGE_EDIT_ENGINE == 'openai':
            ...
            return await finalize(images)
        raise HTTPException(status_code=400, detail='Unsupported image edit engine')
    except asyncio.CancelledError:
        await release_image_billing(billing_context)
        raise
    except HTTPException:
