...
from open_webui.routers.airis.password_reset import router as airis_password_reset_router
from open_webui.routers.airis.telegram_auth import router as airis_telegram_router
...
router = APIRouter()
router.include_router(airis_telegram_router)
router.include_router(airis_password_reset_router)
    ...
    try:
        ...
        if email_service.is_configured():
            try:
                ...
                log.exception('Failed to send signup verification email for user %s', user.id)
        await publish_event(
            request,
            EVENTS.AUTH_SIGNUP,
            actor=user,
