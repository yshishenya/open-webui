...
async def _raise_tts_error(exc: Exception, r=None) -> None:
    ...
    detail = 'Airis: Server Connection Error'
    if r is not None:
        try:
            res = await r.json()
            if 'error' in res:
...
async def speech(request: Request, user=Depends(get_verified_user)):
    ...
    configured_model = await Config.get('audio.tts.model')
    requested_model = payload.get('model')
    billing_model = (
        requested_model if isinstance(requested_model, str) and requested_model else str(configured_model or engine)
    )
    ...
    try:
        billing_context = await preflight_single_rate_hold(
            ...
            lead_magnet_requirements={'tts_seconds': tts_seconds},
        )
    except HTTPException:
        raise
    except Exception as exc:
        log.exception('Audio TTS billing preflight failed: %s', exc)
        raise HTTPException(
    ...
    # Cached audio is still a billable response because it satisfies the request.
    if file_path.is_file():
        try:
            await settle_single_rate_usage(
                billing_context=billing_context,
                measured_units={**measured_units, 'cached': True},
                units=billing_units,
                provider=str(engine),
                reference_type=TTS_HOLD_REFERENCE,
            )
        except Exception:
            log.exception('Failed to settle cached TTS usage')
        await publish_event(
            request,
    ...
    try:
        await settle_single_rate_usage(
            billing_context=billing_context,
            measured_units={**measured_units, 'cached': False},
            units=billing_units,
            provider=str(engine),
            reference_type=TTS_HOLD_REFERENCE,
        )
    except Exception:
        log.exception('Failed to settle TTS usage')
    ...
    try:
        ...
        if r is not None:
            try:
                ...
                detail = f'External: {e}'
        raise Exception(detail if detail else 'Airis: Server Connection Error')
...
async def _transcribe_azure(request, file_path, filename, file_dir, id):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=detail if detail else 'Airis: Server Connection Error',
        )
...
def estimate_stt_seconds(file_path: str) -> int:
    try:
        ...
        log.warning('Failed to read audio duration metadata: %s', exc)

    try:
        audio = AudioSegment.from_file(file_path)
        return max(1, math.ceil(len(audio) / 1000)) if len(audio) > 0 else 0
    except Exception as exc:
        log.warning('Failed to estimate audio duration: %s', exc)
        return 0
...
async def transcription(
    ...
    try:
        ...
        stt_seconds = await asyncio.to_thread(estimate_stt_seconds, file_path)
        if stt_seconds <= 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=ERROR_MESSAGES.FILE_NOT_SUPPORTED,
        ...
        billing_model = str(stt_model or stt_engine or 'whisper')
        try:
            billing_context = await preflight_single_rate_hold(
                user_id=user.id,
                model_id=billing_model,
            ...
            log.exception('Audio STT billing preflight failed: %s', exc)
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail='Billing system temporarily unavailable',
            )
        ...
        try:
            ...
            try:
                await settle_single_rate_usage(
                    ...
                    units=Decimal(stt_seconds),
                    provider=str(stt_engine),
                    reference_type=STT_HOLD_REFERENCE,
                )
            except Exception:
                log.exception('Failed to settle STT usage')
