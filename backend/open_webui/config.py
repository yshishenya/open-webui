...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use the all-in-one Airis Docker container with Ollama
...
# Airis authentication providers
...
YANDEX_OAUTH_SCOPE = os.getenv('YANDEX_OAUTH_SCOPE', 'login:email login:info login:avatar')
YANDEX_REDIRECT_URI = os.getenv('YANDEX_REDIRECT_URI', '')

ENABLE_TELEGRAM_AUTH = os.getenv('ENABLE_TELEGRAM_AUTH', 'False').lower() == 'true'
ENABLE_TELEGRAM_SIGNUP = os.getenv('ENABLE_TELEGRAM_SIGNUP', 'False').lower() == 'true'
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '')
TELEGRAM_BOT_NAME = os.getenv('TELEGRAM_BOT_NAME', os.getenv('TELEGRAM_BOT_USERNAME', ''))
TELEGRAM_BOT_USERNAME = os.getenv('TELEGRAM_BOT_USERNAME', TELEGRAM_BOT_NAME)
TELEGRAM_AUTH_ORIGIN = os.getenv('TELEGRAM_AUTH_ORIGIN', '')
TELEGRAM_AUTH_MAX_AGE_SECONDS = int(os.getenv('TELEGRAM_AUTH_MAX_AGE_SECONDS', '600'))
    ...
    if VK_CLIENT_ID and VK_CLIENT_SECRET:
        ...
        def vk_oauth_register(oauth: OAuth):
            return oauth.register(
                ...
                client_kwargs=oauth_client_kwargs(VK_OAUTH_SCOPE),
    ...
    if YANDEX_CLIENT_ID and YANDEX_CLIENT_SECRET:
        ...
        def yandex_oauth_register(oauth: OAuth):
            return oauth.register(
                ...
                client_kwargs=oauth_client_kwargs(YANDEX_OAUTH_SCOPE),
...
lead_magnet_quotas = DEFAULT_LEAD_MAGNET_QUOTAS.copy()
try:
    configured_lead_magnet_quotas = json.loads(os.getenv('LEAD_MAGNET_QUOTAS', '{}'))
    if isinstance(configured_lead_magnet_quotas, dict):
        for key in DEFAULT_LEAD_MAGNET_QUOTAS:
            value = configured_lead_magnet_quotas.get(key)
            if isinstance(value, int):
                lead_magnet_quotas[key] = value
except json.JSONDecodeError:
    log.warning('Invalid LEAD_MAGNET_QUOTAS JSON, using defaults')

DEFAULT_CONFIG = {
    'billing.lead_magnet.enabled': os.getenv('LEAD_MAGNET_ENABLED', 'False').lower() == 'true',
    'billing.lead_magnet.cycle_days': int(os.getenv('LEAD_MAGNET_CYCLE_DAYS', '30')),
    'billing.lead_magnet.quotas': lead_magnet_quotas,
    'billing.lead_magnet.config_version': 0,
