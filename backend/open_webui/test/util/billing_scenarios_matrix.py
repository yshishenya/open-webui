...
def get_ledger_entry_by_correlation(
    correlation_id: str,
    reference_type: str,
    entry_type: LedgerEntryType,
) -> Optional[LedgerEntry]:
    return (
        Session.query(LedgerEntry)
        .filter(
            LedgerEntry.correlation_id == correlation_id,
            LedgerEntry.reference_type == reference_type,
            LedgerEntry.type == entry_type.value,
        )
        .first()
...
def get_usage_event_by_correlation(correlation_id: str) -> Optional[UsageEvent]:
    return Session.query(UsageEvent).filter(UsageEvent.correlation_id == correlation_id).first()
